...
public class GPTagProvider extends BlockTagsProvider {
	...
	public static class GPItemTagProvider extends ItemTagsProvider {
		...
		@Override
		protected void addTags() {
			this.tag(GPTags.Items.UNDROPPABLE).add(GPBlocks.PACMAN_TROPHY.get().asItem(), GPBlocks.STONE_PACMAN_STATUE.get().asItem(), GPBlocks.GOLDEN_PACMAN_TROPHY.get().asItem(), GPBlocks.DIAMOND_PACMAN_TROPHY.get().asItem(),
					GPItems.RANDOM_ITEMS_DROP_TOOLTIP.get(), GPItems.PACMAN_ICON.get(), GPItems.VULNERABLE_GHOST_ICON.get(), GPItems.FLASHING_VULNERABLE_GHOST_ICON.get(),
					...
					.addOptional(new ResourceLocation("theoneprobe", "creativeprobe"));
		}
	}
}
